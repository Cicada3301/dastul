define(['blocks', 'mathUtils'], function(Blocks, mathUtils){
    var basicLayers={
        generate:function(world){
            var lastY=mathUtils.getRandomInt(world.generation.terrain.max, world.generation.terrain.min);
            var newY=0;
            for(var col=0; col<world.width; ++col){
                ...
                if(!mathUtils.isBetween(lastY+delta, world.generation.terrain.max, world.generation.terrain.min)) delta*=-1;
                newY= lastY+delta;
                lastY= newY;
